    ...
    def display_images(self, visuals, epoch, table=True, phase='train'):
        ...
        if self.display_id > 0:
            if table:
                for i, (label, image_numpy) in enumerate(visuals.items()):
                    if i == 0:
                        ...
                        if 'sem' in label:
                            ...
                            instances.append(image_numpy[::-1,:])
                        else:
                ...
                if instances:
                    self.vis.heatmap(X=np.concatenate(instances, axis=0),
                                    opts=dict(title='{} Epoch[{}] '.format(self.name, epoch) + label), win=self.display_id + idx + 1)
