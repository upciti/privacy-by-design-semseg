...
class BaseModel:
    ...
    def initialize(self, opt):
        ...
        self.input = self.get_variable(
            torch.FloatTensor(
                self.batchSize, 3, self.opt.image_size, self.opt.image_size
            )
        )
        self.target = self.get_variable(
            torch.FloatTensor(
                self.batchSize, 1, self.opt.image_size, self.opt.image_size
            )
        )
        # self.logfile = # ToDo
    ...
    def create_save_folders(self):
        ...
        if self.opt.save_samples:
            ...
            self.save_samples_path = os.path.join(
                "results/train_results/", self.opt.name
            )
            ...
            if self.opt.test:
                self.save_samples_path = os.path.join(
                    "results/test_results/", self.opt.name
                )
                self.save_samples_path = os.path.join(
                    self.save_samples_path, self.opt.epoch
                )
                for subfolder in subfolders:
                    path = os.path.join(self.save_samples_path, subfolder)
                    os.system("mkdir -p {0}".format(path))
